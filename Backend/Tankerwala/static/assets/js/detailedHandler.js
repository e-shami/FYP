$(document).ready(function() {

 $('#Scontract').click(function(){
    var imgSrc = $(this).val();

$("#modal").find('.body').empty();
$("#modal").find("footer").empty();
    var link = "<a href='"+imgSrc+"' target='_blank' class='w-full px-5 py-3 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg sm:w-auto sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple' > Open Image In New Tab </a>"
    var img = "<img src='"+imgSrc+"'>"
    $("#modal").find('.body').append(img)
    $("#modal").find("footer").append(link)
    $('#modalButton').click();
 })
 $('#Scnic').click(function(){
    var imgSrc = $(this).val();

$("#modal").find('.body').empty();
$("#modal").find("footer").empty();
    var link = "<a href='"+imgSrc+"' target='_blank' class='w-full px-5 py-3 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg sm:w-auto sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple' > Open Image In New Tab </a>"
    var img = "<img src='"+imgSrc+"'>"
    $("#modal").find('.body').append(img)
    $("#modal").find("footer").append(link)
    $('#modalButton').click();
 })
});
$(".cnic").click(function(){
    var imgSrc = $(this).val();

$("#modal").find('.body').empty();
$("#modal").find("footer").empty();
    var link = "<a href='"+imgSrc+"' target='_blank' class='w-full px-5 py-3 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg sm:w-auto sm:px-4 sm:py-2 active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple' > Open Image In New Tab </a>"
    var img = "<img src='"+imgSrc+"'>"
    $("#modal").find('.body').append(img)
    $("#modal").find("footer").append(link)
    $('#modalButton').click();

})
function back(win)
{
    if(win=="staff"){
    $('#mainBox').load("staff/").show('')
    }
    else if(win=="mem")
    {
    $('#mainBox').load("members/").show('')
    }
}
